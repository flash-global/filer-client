...
class Filer extends AbstractApiClient implements FilerInterface
{
    ...
    public function uploadByChunks(File $file, $flags = null, callable $fulfilled = null, callable $rejected = null)
    {
        ...
        switch($method) {
            ...
            case 'PATCH':
                return $this->updateFile($file, $method, $uri, $fulfilled, $rejected);
                break;
        }
    }
     ...
     * @return string
     */
    protected function updateFile(File $file, $method, $uri, $fulfilled, $rejected)
    {
        $createFileBody = function (File $file) {
            $transformer = new ContextTransformer();
            $contexts = $file->getContexts();
            /** @var Context $context */
            foreach ($contexts as $k => $context) {
        ...
        try {
            $response = $client->send(
                new Request(
                    ...
                    $uri,
                    ['Content-Type' => 'application/x-www-form-urlencoded; charset=UTF-8'],
                    http_build_query($createFileBody($file))
                )
            );
